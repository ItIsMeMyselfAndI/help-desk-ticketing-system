...
# read
class TestDBReadAttachment(unittest.TestCase):
        ...
        for arg in invalid_args:
            ...
            with self.subTest(arg=arg, attachment_id=attachment_id):
                with self.assertRaises(pydantic.ValidationError):
                    crud.get_attachment_good(arg, attachment_id)
            # attachment id
            with self.subTest(arg=arg):
                with self.assertRaises(pydantic.ValidationError):
                    crud.get_attachment_good(self.db, arg)
    ...
    def test_left_out_of_bound_id(self):
        ...
        result_attachment, status_code = crud.get_attachment_good(
    ...
    def test_correct_id(self):
        ...
        result_attachment, _ = crud.get_attachment_good(self.db, attachment_id)
        if result_attachment is None:
            self.fail("read attachment failed")
        result_attachment_dict = result_attachment.model_dump()

        self.assertEqual(
            result_attachment_dict["ticket"]["id"],
