...
class TestDBDeleteTicket(unittest.TestCase):
        ...
        for arg in invalid_args:
            ...
            with self.subTest(arg=arg, ticket_id=ticket_id):
                with self.assertRaises(pydantic.ValidationError):
                    crud.delete_ticket(arg, ticket_id)
            # ticket id
            with self.subTest(arg=arg):
                with self.assertRaises(pydantic.ValidationError):
                    crud.delete_ticket(self.db, arg)
    ...
    def test_left_out_of_bound_id(self):
        ...
        result_ticket, status_code = crud.delete_ticket(self.db, ticket_id - 100)
    ...
    def test_correct_id(self):
        ...
        result_ticket, _ = crud.delete_ticket(self.db, ticket_id)
        if result_ticket is None:
            self.fail("delete ticket failed")
        result_ticket_dict = result_ticket.as_dict()

        self.assertEqual(
            result_ticket_dict["issuer_id"], self.existing_ticket_dict["issuer_id"]
        )
        self.assertEqual(
            result_ticket_dict["assignee_id"], self.existing_ticket_dict["assignee_id"]
        )
        self.assertEqual(
